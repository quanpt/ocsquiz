...
CREATE VIEW "FullQuizes" AS SELECT quizId, title, COUNT(questionId) AS QuestionCount, SUM(CASE WHEN answer IS NULL THEN 0 ELSE 1 END) AS AnswerCount, SUM(CASE WHEN UPPER(answer) == UPPER(providedAnswer) THEN 1 ELSE 0 END) AS CorrectAnswerCount FROM FullAnswers GROUP BY quizId
